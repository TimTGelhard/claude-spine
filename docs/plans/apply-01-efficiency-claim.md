...
- [x] `CLAUDE.md:42` + `:44` reframed: "prove the win" / "Cheaper is the answer" → bounded-per-load-cost + routing-accuracy; the stale "an actual baseline run is the LC1 launch task in FIXES" wording corrected (LC1 ran → cite REPORT.md).
- [x] `README.md` + `landing/index.html` + `EXPLAINER.md` swept for any "fewer tokens than vanilla / cheaper / saves" comparative-reduction claim; each reframed or confirmed already-safe and noted.
- [x] `FIXES.md` A1 "LC1 EXECUTED" + "A1 extended" blocks compressed to action lines + a pointer to audit-02 F0–F5 (CLAUDE.md rule 1 — FIXES is an action queue, not a results essay); A1 marked resolved.
- [x] `CHANGELOG.md` `[Unreleased]` gains one slim Changed bullet; `tests/run.sh` still passes (the `run.sh` edit caused no regression).
...
**Status**: `done` (2026-05-29) — claim #1 reframed on the live surface (`CLAUDE.md:42/44`: "prove the win"/"cheaper" → **bounded per-load cost + routing accuracy**, cites REPORT.md); README:92 / landing / EXPLAINER swept = already in the safe lean-on-demand framing (no edit); `FIXES.md` A1 compressed to an action-shaped **✅ RESOLVED** entry + LC1 marked done; `CHANGELOG.md` `[Unreleased]` Changed bullet; `tests/run.sh` green (7 suites). All 4 verify checks passed.
...
- **apply-01 Session 2 done 2026-05-29 — docs-only, no API spend.** Reframed claim #1 at `CLAUDE.md:42/44` ("prove the win"/"cheaper" → bounded per-load cost + routing accuracy, cites REPORT.md); swept README:92/landing/EXPLAINER (already safe — no edit, confirms audit-05); compressed + **resolved** `FIXES.md` A1 (two ~15-line narrative essays → one action entry, rule 1) + marked LC1 done + dropped A1 from FIXES/CHANGELOG residue lists; `CHANGELOG [Unreleased]` Changed bullet; `tests/run.sh` green (7 suites). All 4 verify checks passed. Pending-notes block reviewed → 3 entries all dismissed (2 PF2 cue self-captures + 1 stray heartbeat; the `aggregate.py` Totals finding already lives in § Cross-session notes + PROGRESS Blockers). Session 1 committed separately (`3ee2c92`). **Section apply-01 complete** → pointer to apply-02 (needs `/prep`).
