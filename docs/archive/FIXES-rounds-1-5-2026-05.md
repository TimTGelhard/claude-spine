...
> **Archived 2026-05-28** as part of the FIXES restructure. This is the full per-round audit history (Pass 1 drift catalog → Pass 2 pillar findings → Pass 3 second senior review → Pass 4 global-readiness → Pass 5 folded BIAS-AUDIT residue → Pre-launch checklist) with every `[shipped]` annotation intact. Kept here so future readers can see the *reasoning* behind shipped items + the *evidence trail* for each round. **The live FIXES.md at the repo root now lists only currently-open items** (~80 lines). Per-round shipping narrative also lives in [`CHANGELOG.md`](../../CHANGELOG.md) under `[Unreleased]` and `[0.10.0]`. **Do not file new fixes here** — file them in the live `FIXES.md`.
...
# Pre-launch checklist — folded from the archived LAUNCH.md (2026-05-28)

> The original `LAUNCH.md` at the repo root has been archived to
> [`docs/archive/LAUNCH-pre-1.0.md`](docs/archive/LAUNCH-pre-1.0.md).
> Almost every phase shipped in May 2026; the residue is below. All
> remaining items are manual user-actions (domain registration, asset
> creation, verification reruns) — no further code work is needed inside
> the spine to unblock launch.

### LC1. L4c — Token-efficiency benchmark baseline run

Harness + eval-set shipped 2026-05-28. The actual benchmark run
(spine-on vs spine-off across the eval set) is gated on authorization
because the spend is non-trivial (~$9–$15 of Sonnet 4.6 per full run,
per the original L4c notes — adjust to current Anthropic pricing when
running). **Owner:** Tim. **Output:** a token-efficiency number for
LC4's demo.

### LC2. L7a — Domain registration

Decide and register the canonical domain. Update `landing/index.html`
canonical URL + GitHub social-preview URL once known. **Owner:** Tim.
No code work until the domain string is fixed.

### LC3. L7c — Waitlist / signal-of-interest path

A simple "follow / star / waitlist email" surface so launch traffic
has somewhere to land. Could be as light as a single mailto + GitHub
star prompt on the landing page, or as heavy as a real waitlist form.
**Owner:** Tim. **Decision needed** before LC5.

### LC4. L7d — Demo

A 60–90 second screencast showing the spine in action: `/onboard`
walkthrough → `/prep` a sample project → `/done` close. LC1's
benchmark number goes alongside as evidence. **Owner:** Tim.
**Blocked by:** LC1 (numbers) and LC2 (domain in the URL bar of the
screencast).

### LC5. L7e — Public launch

Generate the `og:image` (the TODO marker already lives in
`landing/index.html` `<head>`), then publish on the chosen channel(s).
The archived LAUNCH.md L7b notes have the technical detail; LC2–LC4
are the prerequisites. **Owner:** Tim.

### LC6. L8 — Personal-migration verification

The `~/.claude` → spine-installed handoff for Tim's personal setup
shipped 2026-05-27 but the verification steps (restart Claude Code,
run `/onboard`, fresh-session "are my defaults still right?" check)
are pending. **Owner:** Tim. **Effort:** ~15 minutes when picked up.

---

## Pre-launch checklist — order

LC1 (benchmark) ➜ LC2 (domain) ➜ LC3 (waitlist) ➜ LC4 (demo, needs
LC1 + LC2) ➜ LC5 (launch, needs LC2–LC4) ➜ LC6 (verification can
happen anytime, no dependencies). Once all six close, v1.0 ships, the
spine becomes canonically launched, and the archived LAUNCH-pre-1.0
doc + this checklist both become purely historical.
