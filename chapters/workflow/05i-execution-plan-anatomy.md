...
- **End-of-session.** `/session-end` (or manual writeback per [05j](05j-cold-start-protocol.md)) updates the session status, adds cross-session notes if anything was discovered, updates `PROGRESS.md`.
...
- **Templates keep entries uniform** — `/session-start` knows exactly where to look in any session entry, regardless of which section.
