...
| Field | Read by | What changes when it changes |
|---|---|---|
| **Explanation style** (set via `/explain` — no onboarding question) | Every response (read implicitly by Claude's response policy); surfaced + written by the `/explain` command | Simple / Standard / Detailed. "Simple" → plain language, minimal jargon, *what* changed + *why* in a line or two, deeper detail on request, **and always closes with one clearly-recommended next action stated as a concrete step** (e.g. "open a fresh terminal and run /prep"), alternatives demoted to one line. "Detailed" → full technical register, trade-offs spelled out. A distinct axis from **Answer length** (how much text) and **Reasoning depth** (how much reasoning is shown): this controls register / plainness. Defaults to Standard; change with `/explain <level>` or by hand-editing the line. |
