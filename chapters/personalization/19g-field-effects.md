...
| **VCS host** (Q8) | `op-onboard/handoff.md` (suggests the matching CLI), every chapter that mentions PRs / MRs (workflow chapter 05, anti-patterns 18g) | CLI suggestions (`gh` vs `glab` vs `bb`), terminology ("pull request" vs "merge request"), and URL patterns for cross-references. Note: the spine surfaces the CLI as a *suggestion* in the handoff; it does not auto-add the CLI to your Bash allowlist — copy the suggested line into `~/.claude/settings.json` to grant permission. |
| **Plans dir** (G2) | `op-spine-active`, `spine-writeback.sh` | Overrides the four built-in plan-layout conventions (`docs/plans/`, `docs/specs/`, `plans/`, `specs/`). If you set `Plans dir: project/x/plans`, ambient cold-start and writeback both honor that path. A project-level `CLAUDE.md` line `Plan layout: <dir> <progress-file>` overrides the profile value per-project. |
...
| Field | Read by | What changes when it changes |
|---|---|---|
| **Push-back intensity** (Q4) | `op-signaling` (11 — phrasing of flags), all chapters with "Claude objects" language | "Soft" → "Worth checking: …"; "Standard" → "Hold on — …"; "Spar with me" → "No. Here's why: …". Same flags fire; only delivery changes. |
